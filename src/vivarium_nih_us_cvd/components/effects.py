...
class PolypillEffect:
    """A component to model the impact of the polypill risk on medication adherence levels"""

    def __repr__(self):
        return "PolypillEffect"
    ...
    def setup(self, builder: Builder) -> None:
        ...
        self.simulation_start_time = get_time_stamp(builder.configuration.time.start)
        self.randomness = builder.randomness.get_stream(self.name)
        self.exposure = builder.value.get_value(data_values.PIPELINES.POLYPILL_EXPOSURE)
        self._register_target_modifiers(builder)
